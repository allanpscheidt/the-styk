...
- App de barra de menus sem Dock: o usuário abre o The Styk e **nada visível acontece**. Trava real de descoberta.
- **[NOVO — mínimo]** Alerta único no primeiro launch (flag em UserDefaults, mesmo padrão do `-1743`): título "O The Styk está na barra de menus", texto curto (ver §2). Nada de tour, nada de janela de boas-vindas.
...
- Ponto verificado: o painel é `nonactivating`, então clicar/digitar na nota **não** ativa o The Styk — o Finder segue frontal e o timer continua. Sem bug de "nota some quando clico nela".
...
- `NSStatusItem.button` → label "The Styk". `NSTextView` → label "Texto da nota".
...
| Chave | Texto |
|---|---|
| `NSAppleEventsUsageDescription` | `O The Styk pergunta ao Finder qual pasta está aberta, para mostrar cada nota no lugar certo.` |
| `CFBundleName` | `The Styk` |
...
| Apagar nota | `Apagar…` |
| Sobre | `Sobre o The Styk` |
| Sair | `Sair do The Styk` |

### Alertas
**Primeiro launch** (uma vez):
- Título: `O The Styk está na barra de menus`
...
- Título: `O The Styk precisa falar com o Finder`
- Texto: `Sem essa permissão, o The Styk não sabe qual pasta está aberta. Ative em Ajustes do Sistema → Privacidade e Segurança → Automação → The Styk → Finder.`
...
| Share | `Exportar nota` |
| Lixeira | `Apagar nota` |
| Status item (VoiceOver) | `The Styk` |
...
| Nome de arquivo exportado | `The Styk – <snippet>.txt` · fallback `The Styk – Nota.txt` |
